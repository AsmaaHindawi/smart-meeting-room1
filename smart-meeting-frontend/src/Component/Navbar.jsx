...
function Navbar() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { setShowContactForm(false); setStatus(""); }, 2000);
    } catch (error) {
  ...
  const profilePath = user ? getDashboardPath(user.roles) : "/";
  ...
  return (
    <>
      ...
      <nav className="relative flex items-center justify-between px-10 py-4 shadow-md bg-white overflow-hidden">
        <Link to="/">
          <div className="z-10">
            <img src="/Images/logoMeetSM.png" alt="SmartMeet Logo" className="h-20 max-w-30 ml-20" />
          </div>
        </Link>
        ...
        >
          <li><a href="#about" className="hover:text-[#7d64fb] transition duration-200">About Us</a></li>
          <li><a href="#features" className="hover:text-[#7d64fb] transition duration-200">Features</a></li>
          <li>
            <button onClick={() => setShowContactForm(true)} className="hover:text-[#7d64fb] transition duration-200">
              Contact Us
            </button>
          </li>
        </ul>
          ...
          {!loading && user ? (
            <>
              ...
              <button
                ...
                className="bg-[#2c2e5f] text-white px-5 py-2 rounded-full hover:bg-[#7d64fb] transition font-medium"
              >
                Sign Out
              </button>
            </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm">
          <div className="relative bg-white rounded-3xl w-full max-w-lg shadow-2xl p-8 mx-4">
            <div className="absolute top-4 right-4">
              <button onClick={() => setShowContactForm(false)} className="text-gray-400 hover:text-red-500 text-2xl font-bold">
                &times;
              </button>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="w-full py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-semibold hover:from-indigo-600 hover:to-indigo-700 transition">
                Send Message
              </button>
